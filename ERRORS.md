...
1.3. This appears when the IdP the user has used does not release the needed
...
1.4. This is displayed when the user has not added a displayName to their
...
1.12. This is shown in the notifications area when a user tries to create an
...
1.13. Message shown in the notifications area when the user is trying to send a
...
1.15. When a user edits an invitation to sign, it may be necessary to send
...
1.16. When a user tries to cancel an invitation they had made, and there is an
...
1.17. When a user tries to cancel an invitation they have made, and there is
...
1.18. When a user tries to preview a document they have been invited to sign,
and the document is not found in the backend, they are shown this message in
the notifications area.

    Cannot find the document being signed

1.19. When a user has been invited to sign a document, and tries to decline the
...
1.20. When a user tries to fill in a PDF form, previous to adding signatures to
it, and the process fails, they are shown this in the notifications area.

    Problem filling in form in PDF, please try again

1.21. When a user tries to edit an invitation they have made, and one of the
...
2.1. The invitation form will not validate if there are 2 invitees with the
same email, and the field with the 2nd ocurrence will show this error message:

    That email has already been invited

2.2. The invitation form will not validate if one of the invitees is the actual
user who is making the invitation, showing this message in the form field with
the email.

    Do not invite yourself

2.3. When the procedure to read the previous signatures of a document fails,
...
2.5. There is a limit on the number of invitees for each invitation, imposed by
...
2.6. When there is an unidentified error loading a document, this error is
shown in the notifications area.

    Error loading {document_name}

2.7. In the same case as for 2.6, this is shown on the box representing the
document that failed to load.

    Document could not be loaded

2.8. When a user tries to load a document that is neither PDF nor XML, this message is shown in the notifications area.

    Not a PDF or XML document: {document_name}

2.9. When the user does not provide the required LoA for an invitation, the
...
2.10. When the document has failed to load for any reason, the contextual help
...
2.11. When the API has failed to prepare a document for signing, for whatever
...
2.12. When there was a problem signing the document, the contextual help on the
...
2.15. When there was a problem signing the document, this message is shown in
...
2.16. There are several ways in which documents are examined, and each may result
...
2.18 When tries to sign a document that exceeds the max allowed size, and the
...
2.19. When a user tries to decline an invitation and the procedure fails, this
is shown in the notifications area.

    Problem declining signature

2.19. When a user tries to edit an invitation they had made and the procedure
fails, this is shown in the notifications area.

    Problem editing invitation to sign, please try again

2.19. When a user tries to preview a document that they have or have been
...
2.20. When a user tries to edit an invitation, the app tries to lock the
...
2.21. When a user tries to remove an invitation they have made, anthe process
fails, this is shown in the notifications area.
...
2.23. When a user tries to send invitations to sign, and the process fails,
this is shown in the notifications area.

    Problem sending invitations to sign, please try again

2.23. When a user tries to skip the final signature in an invitation they have
made, and the process fails, this is shown in the notifications area.

    Problem skipping final signature, please try again

2.23. When a user tries to fill in a PDF form, and the process fails, this is
shown in the notifications area.

    Problem filling in PDF form, please try again

2.24. When a user tries to load a document, and there are problems storing them
...
2.24. When a user tries to load a document, and there are problems after having
