...
test('Sign one test PDF document', async ({ browser }) => {
  ...
  await fileChooser.setFiles(path.join(__dirname, 'fixtures/corrupted.pdf'));

  await expect(page.locator('[id="local-doc-corrupted\\.pdf"]')).toContainText('Document seems corrupted');
