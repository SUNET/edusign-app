...
back-init-msgs:
	@cd $(BACK_DIR); \
    uv run pybabel extract -F src/edusign_webapp/babel.cfg -o src/edusign_webapp/messages.pot ./src/ ; \
	uv run pybabel init -i src/edusign_webapp/messages.pot -d src/edusign_webapp/translations -l en ; \
	uv run pybabel init -i src/edusign_webapp/messages.pot -d src/edusign_webapp/translations -l sv ; \
	uv run pybabel init -i src/edusign_webapp/messages.pot -d src/edusign_webapp/translations -l es
...
back-extract-msgs:
	@cd $(BACK_DIR); \
    uv run pybabel extract -F src/edusign_webapp/babel.cfg -o src/edusign_webapp/messages.pot ./src/ ; \
	uv run pybabel update -i src/edusign_webapp/messages.pot -d src/edusign_webapp/translations

## Compile translatable messages
.PHONY: back-compile-msgs
back-compile-msgs:
	@cd $(BACK_DIR); \
	uv run pybabel compile -f -d src/edusign_webapp/translations

## Reformat Python sources
.PHONY: back-reformat
back-reformat:
	@cd $(BACK_DIR); \
	uv run isort --line-width 120 --atomic --project edusign-webapp $(BACK_SOURCE) ; \
	uv run black --line-length 120 --target-version py38 --skip-string-normalization $(BACK_SOURCE)

## Type check Python sources
.PHONY: back-typecheck
back-typecheck:
	@cd $(BACK_DIR); \
		uv run mypy --ignore-missing-imports $(BACK_SOURCE)

## Test Python code
.PHONY: back-test
back-test:
	@cd $(BACK_DIR); \
		uv run pytest --log-cli-level DEBUG $(BACK_SOURCE) ; \
